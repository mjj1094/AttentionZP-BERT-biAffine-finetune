    ...
    for p in paths:
        ...
        if file_name.endswith("onf"):
            ...
            for k in nodes_info:
                ...
                orig_to_tok_maps_bert.append(orig_to_tok_map)#orig_to_tok_map=[0,3,4,6]
                indexed_tokens = tokenizer.convert_tokens_to_ids(bert_tokens)#bert_tokens=['什', '么', '样', '的', '记', '忆', '？']
                vectorized_sentences_bert_idx.append(indexed_tokens)#indexed_tokens=[784, 720, 3416, 4638, 6381, 2554, 8043]
...
def generate_vector_data(file_name="",test_only=False):
    ...
    if not test_only:
        # train_zp_info, train_vectorized_sentences,train_vectorized_sentences_bert,train_orig_to_tok_maps_bert,train_mask_sentences_bert,train_vectorized_sentences_bert_idx = generate_vector(DATA+"train/"+file_name,args.data+"train_list")
        train_zp_info, train_vectorized_sentences, train_orig_to_tok_maps_bert , train_vectorized_sentences_bert_idx= generate_vector( DATA + "train/" + file_name, args.data + "train_list")
        ...
        train_vec_path = train_data_path + "sen_bert_idx.npy"
        numpy.save(train_vec_path, train_vectorized_sentences_bert_idx)
    ...
    # test_zp_info, test_vectorized_sentences,test_vectorized_sentences_bert,test_orig_to_tok_maps_bert,test_mask_sentences_bert,test_vectorized_sentences_bert_idx = generate_vector(DATA+"test/"+file_name,args.data+"test_list")
    test_zp_info, test_vectorized_sentences, test_orig_to_tok_maps_bert,test_vectorized_sentences_bert_idx = generate_vector(
        DATA + "test/" + file_name, args.data + "test_list")
    ...
    test_vec_path = test_data_path + "sen_bert_idx.npy"
    numpy.save(test_vec_path, test_vectorized_sentences_bert_idx)
...
def generate_vec(data_path):
    ...
    vectorized_sentences_bert_idx = numpy.load(data_path + "sen_bert_idx.npy")
    ...
    for zp,candi_info in zp_info_test:
        index_in_file, sentence_index, zp_index, ana = zp#zp=(14, 14, 11, 1)
        if ana == 1:
            sequence_bert_idx = vectorized_sentences_bert_idx[index_in_file]#sequence_bert_idx: [738, 2218, 1762, 6821, 671, 2399, 8024, 3189, 3315, 1092, 7339, 1169, 2137, 749, 103, 6206, 1762, 671, 736, 1724, 7439, 2399, 2399, 2419, 1184, 8024, 6813, 6862, 6833, 886, 704, 1744, 782, 3696, 2235, 3302, 4638, 5032, 4526, 511]
            sequence_bert_idx.append(102)#add SEP : [738, 2218, 1762, 6821, 671, 2399, 8024, 3189, 3315, 1092, 7339, 1169, 2137, 749, 103, 6206, 1762, 671, 736, 1724, 7439, 2399, 2399, 2419, 1184, 8024, 6813, 6862, 6833, 886, 704, 1744, 782, 3696, 2235, 3302, 4638, 5032, 4526, 511, 102]
            # max_index_bert = len(sequence_bert_idx)
            # sequence_bert_idx = sequence_bert_idx[:min(args.max_sent_len, max_index_bert)]
            # sequence_bert_mask = (len(sequence_bert_idx) * [1] + (args.max_sent_len - len(sequence_bert_idx)) * [0])
            # sequence_bert_idx = (sequence_bert_idx + (args.max_sent_len - len(sequence_bert_idx)) * [0])
                        
            sequence_orig_to_tok_bert = [orig_to_tok_bert[index_in_file]]#no [CLS],[SEP]: [[0, 1, 2, 3, 4, 5, 6, 7, 9, 11, 13, 14, 15, 16, 17, 22, 24, 25, 26, 28, 30, 32, 34, 36, 37, 39]]
            # tmp_input_bert = [sequence_bert_idx]
            i=index_in_file-1
            while i>=max(0, index_in_file - 2):
                tmp_sequence_bert_idx = vectorized_sentences_bert_idx[i]
                tmp_sequence_bert_idx.insert(0,102)
                tmp_sequence_bert_idx.extend(sequence_bert_idx)
                sequence_orig_to_tok_bert.append(orig_to_tok_bert[i])

                # sequence_bert_idx = vectorized_sentences_bert_idx[i]
                # max_index_bert = len(sent_bert_idx)
                # sent_bert_idx = sent_bert_idx[:min(args.max_sent_len, max_index_bert)]
                # sequence_bert_idx = (len(sent_bert_idx) * [1] + (args.max_sent_len - len(sent_bert_idx)) * [0])
                # sequence_bert_mask = (sent_bert_idx + (args.max_sent_len - len(sent_bert_idx)) * [0])
                # sequence_orig_to_tok_bert = (orig_to_tok_bert[index_in_file] + (args.max_sent_len - len(sequence_bert_idx)) * [0])
                # tmp_input_bert.append(sequence_bert_idx)

                # assert sent_bert_idx[0] == 101
                # sent_bert_idx[0]=102
                # sequence_orig_to_tok_bert.extend((orig_to_tok_bert[i] + (args.max_sent_len - len(sent_bert_idx)) * [0]))#是句子内的相对位置!!!!!!!!
                # sequence_bert_idx.extend(sent_bert_idx)
                # sequence_bert_mask.extend(sent_bert_mask)
                i-=1
            tmp_sequence_bert_idx.insert(0, 101)
            too_large=0
            if len(tmp_sequence_bert_idx)>768:
                too_large =len(tmp_sequence_bert_idx)-768
                tmp_sequence_bert_idx=tmp_sequence_bert_idx[-768:]

            # The mask has 1 for real tokens and 0 for padding tokens. Only real
            # tokens are attended to.
            input_mask = [1] * len(tmp_sequence_bert_idx)
            # Zero-pad up to the sequence length.
            padding = [0] * (args.max_seq_length - len(tmp_sequence_bert_idx))
            tmp_sequence_bert_idx += padding
            input_mask += padding


            assert len(tmp_sequence_bert_idx) == args.max_seq_length#nnargs[""]
            assert len(input_mask) == args.max_seq_length

            zp_np_sent_bert.append(tmp_sequence_bert_idx)
            zp_np_sent_bert_mask.append(input_mask)

            zp_index += len(tmp_sequence_bert_idx)-len(sequence_bert_idx)+sequence_orig_to_tok_bert[0][zp_index]#前面所加字数（1-2句）+原句内字序
            idx_dict={0:len(tmp_sequence_bert_idx)-len(sequence_bert_idx),1:len(vectorized_sentences_bert_idx[i+1])+2-too_large,2:1-too_large}#前面所加字数（1-2句），未加原句内字序
            # zp_np_sent_bert.append(tmp_input_bert)
            # zp_sent_bert.append(vectorized_sentences_bert[index_in_file])
            # zp_np_sent_bert.append(sequence_bert_idx)
            ...
            for candi_index_in_file,candi_sentence_index,candi_begin,candi_end,res,target,ifl in candi_info:
                ...
                # candi_vec_index_inside.append((candi_vec_index,candi_sentence_index, candi_begin, candi_end, res, target))

                candi_begin = idx_dict[sentence_index-candi_sentence_index]+sequence_orig_to_tok_bert[sentence_index-candi_sentence_index][candi_begin]#前面所加字数（1-2句）+原句内字序
                candi_end = idx_dict[sentence_index - candi_sentence_index] + \
                              sequence_orig_to_tok_bert[sentence_index - candi_sentence_index][
                                  candi_end]  # 前面所加字数（1-2句）+原句内字序
                candi_vec_index_inside.append((candi_vec_index, zp_vec_index, candi_begin, candi_end, res, target))
                ifl_vecs.append(ifl)
                candi_vec_index += 1

            # zp_candi_target.append((zp_vec_index,candi_vec_index_inside)) #(zpi,candis(candij,res,target))
            # zp_candi_target.append(((zp_vec_index, sentence_index,zp_index), candi_vec_index_inside))
    ...
    save_f = open(data_path + "zp_np_sent_bert", 'wb')
    pickle.dump(zp_np_sent_bert, save_f, protocol=pickle.HIGHEST_PROTOCOL)
    save_f.close()

    save_f = open(data_path + "zp_np_sent_bert_mask", 'wb')
    pickle.dump(zp_np_sent_bert_mask, save_f, protocol=pickle.HIGHEST_PROTOCOL)
    save_f.close()
