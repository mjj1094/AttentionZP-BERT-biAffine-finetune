...
class BertConfig(object):
    ...
    def __init__(self,
                 ...
                 max_position_embeddings=768,#512,
                 type_vocab_size=2,
...
class Network(PreTrainedBertModel):
    def __init__(self, config, hidden_dimention=None, output_dimention=None):
    ...
    def get_bert_output(self,data,input_ids,attention_mask):
        # get bert idx by sent_bert
        # zp_np_sent_bert=torch.tensor(data["zp_np_sent_bert"]).type(torch.cuda.FloatTensor)

        sequence_output, _ = self.bert(input_ids, attention_mask,token_type_ids=None,
                                       output_all_encoded_layers=False)
        zp_i = -1
        for j, zp_reidx in enumerate(data["zp_reindex"]):  # for each zp
            if zp_i != zp_reidx:
                sequence_outputs = []
                for i, (sequence_bert_idx, sequence_orig_to_tok_bert) in enumerate(data["zp_np_sent_bert"][j]):
                    sequence_output, _ = self.bert(input_ids,attention_mask, output_all_encoded_layers=False)

                    sequence_outputs.append(sequence_output.squeeze(1)[:, :256])
                    if i == 0:
                        pre_same_zp = sequence_output.squeeze(1)[data["zp_idx"][j]][:512].unsqueeze(0)
                        if j == 0:
                            zps_sent_cls_output_bert = pre_same_zp
                        else:
                            zps_sent_cls_output_bert = torch.cat((zps_sent_cls_output_bert, pre_same_zp), 0)
                        zp_i += 1
                # candis_bert=torch.empty((1,40,256)).type(torch.cuda.FloatTensor)
                for i, candi_sent_idx in enumerate(data["candi_sent_idx"]):
                    candi_bert = sequence_outputs[data["zp_sent_idx"][i] - candi_sent_idx][
                        data["candis_begin"][i]].unsqueeze(0)
                    for idx in range(data["candis_begin"][i] + 1, data["candis_end"][i] + 1):
                        candi_bert = torch.cat((candi_bert, sequence_outputs[data["zp_sent_idx"][i] - candi_sent_idx][idx].unsqueeze(0)),0)
                    if len(candi_bert) >= 40:  # 限制candi长度max为8*5
                        candi_bert = candi_bert[:, -40:]
                    candi_bert = torch.cat(
                        (torch.zeros((40 - len(candi_bert), 256)).type(torch.cuda.FloatTensor), candi_bert), 0)
                    if i == 0:
                        candis_bert = candi_bert.unsqueeze(0)
                    else:
                        candis_bert = torch.cat((candis_bert, candi_bert.unsqueeze(0)), 0)
            else:
                zps_sent_cls_output_bert = torch.cat((zps_sent_cls_output_bert, pre_same_zp), 0)
        return zps_sent_cls_output_bert,candis_bert
    def get_model_output(self,data,zps_sent_cls_output_bert,candis_bert):
        ...
        nps = []
        # for npt, zpt in zip(torch.transpose(candi_bert,0,1), zp_representation):  # 40*x*256,x*512
        for npt, zpt in zip(candis_bert, zps_sent_cls_output_bert):  # x*40*256,x*512
        ...
        return zps_sent_cls_output_bert, candi_representation, feature
    def forward(self,data,dropout=0.0):
        zp_np_sent_bert=torch.tensor(data["zp_np_sent_bert"]).type(torch.cuda.LongTensor)
        zp_np_sent_bert_mask = torch.tensor(data["zp_np_sent_bert_mask"]).type(torch.cuda.LongTensor)

        # zps_sent_cls_output_bert, candis_bert = checkpoint(self.get_bert_output, data)
        # candi_representation, feature = checkpoint(self.get_model_output, data, zps_sent_cls_output_bert, candis_bert)
        zps_sent_cls_output_bert, candis_bert = self.get_bert_output(data,zp_np_sent_bert,zp_np_sent_bert_mask)
        candi_representation, feature = self.get_model_output(data,zps_sent_cls_output_bert,candis_bert)
        output, softmax_out = self.generate_score_bert_biaffine(zps_sent_cls_output_bert, candi_representation, feature)
